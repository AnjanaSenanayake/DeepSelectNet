...
def export_numpy(reads_array, reads_array_temp):
    ...
    for fast5_file in files:
        ...
        if fast5_file.endswith(".fast5"):
            with get_fast5_file(file_path, mode="r") as f5:
                ...
                if 2*segment_count % BATCH == 0:
                    export_numpy(reads_array, reads_array_temp)
                    reads_array = []
                    reads_array_temp = []
...
def main(fast5_dir, cutoff, subsample_size, sampling_coefficient, batch, pico, output):
    global FAST5_DIR, CUTOFF, SUB_SAMPLE_SIZE, SAMPLING_C0, BATCH, IS_PICO, OUTPUT
    FAST5_DIR = fast5_dir
    CUTOFF = cutoff
